...
use fuzzforge::{
    ...
    RunConfig, RunSession, Store, generate_seed, hash_wasm_file, parse_wasm_metadata,
...
fn submit_proof(
    ...
    let metadata = parse_wasm_metadata(&wasm).context("failed to parse WASM metadata")?;
    let github_token = match metadata.github_repository.as_deref() {
